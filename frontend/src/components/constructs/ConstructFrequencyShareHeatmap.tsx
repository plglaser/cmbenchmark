...
interface ConstructFrequencyShareHeatmapProps {
  data: Array<{
    modelId: string;
    relpath: string;
...
export function ConstructFrequencyShareHeatmap({
  data,
  constructCatalog,
  constructTotals,
  maxConstructs = 60,
}: ConstructFrequencyShareHeatmapProps) {
  ...
  if (!data || data.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Construct × Model (Share)</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-muted-foreground">No construct share data available.</p>
        </CardContent>
      </Card>
    );
  }
    ...
    () =>
      [...data]
        .map((item) => ({
          ...
          sharesByConstruct: item.sharesByConstruct || {},
        }))
  ...
  const constructs = useMemo(() => {
    ...
    if (totals.length > 0) {
      ...
      for (const item of models) {
        Object.keys(item.sharesByConstruct || {}).forEach((cid) => constructSet.add(String(cid)));
      }
  ...
  const maxShare = useMemo(() => {
    ...
    for (const model of visibleModels) {
      for (const construct of visibleConstructs) {
        const value = Number(model.sharesByConstruct?.[construct.id] || 0);
        if (value > maxValue) {
          maxValue = value;
        }
      }
    ...
    <Card>
      ...
      <CardContent>
        <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
          ...
          <div>
            Max share: {maxShare > 0 ? `${(maxShare * 100).toFixed(1)}%` : '—'} ·{' '}
            {kindFilterLabel(kindFilter)}
          </div>
          {(hasMoreModels || hasMoreConstructs) && (
            <span>
              Showing {visibleConstructs.length} of {totalConstructs} constructs and {pageStart}-{pageEnd} of{' '}
              {totalModels} models.
            </span>
          )}
        </div>
            ...
            <TableBody>
              {visibleModels.map((model) => (
                <TableRow key={model.modelId}>
                  ...
                  {visibleConstructs.map((construct) => {
                    const share = Number(model.sharesByConstruct?.[construct.id] || 0);
                    const intensity = maxShare > 0 ? share / maxShare : 0;
                    const alpha = share > 0 ? 0.2 + 0.8 * intensity : 0;
                    const shareLabel = `${(share * 100).toFixed(2)}%`;
                    const title = `${construct.label} in ${model.relpath}: ${shareLabel}`;
                    return (
                      <TableCell key={`${model.modelId}:${construct.id}`} className="p-1">
                        <div className="group relative mx-auto h-4 w-4" title={title}>
                          <div
                            className={`h-4 w-4 rounded-sm ${share > 0 ? '' : 'bg-muted border'}`}
                            style={share > 0 ? { backgroundColor: `rgba(59, 130, 246, ${alpha})` } : undefined}
                          />
                          {share > 0 && (
                            <div className="pointer-events-none absolute left-1/2 top-[-6px] z-10 hidden -translate-x-1/2 -translate-y-full rounded bg-background px-2 py-1 text-[10px] text-foreground shadow-md ring-1 ring-border group-hover:block">
                              {shareLabel}
                            </div>
                          )}
                        </div>
